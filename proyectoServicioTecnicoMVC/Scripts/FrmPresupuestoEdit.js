...
function pintar(objConfiguracion, objBusqueda) {
    ...
    var urlAbsolutaRE = window.location.protocol + "//" + window.location.host + raiz + "ReparacionPresupuesto/lista/?id=" + document.getElementById("txtidRecepcion").value;
    ...
    Promise.all([fetch(urlAbsoluta), fetch(urlAbsolutaRE)])
        ...
        .then(results => {
            ...
            contenido += generarTablaR(objConfiguracion, res, resRE)
            contenido += "</div>";
...
var obs ;
var cant ;
function generarTablaR(objConfiguracion, res, resRE) {
    var contenido = "";
    ...
    for (var i = 0; i < res.length; i++) {
        ...
        for (var x = 0; x < resRE.length; x++) {
            filaRE = resRE[x]
            if (fila["ID_Reparacion"] == filaRE["ID_Reparacion"]) {
                console.log(resRE[x])
                contiene = 1;
                this.obs = filaRE["Observacion"];
                this.cant = filaRE["Cantidad"];
        ...
        if (objConfiguracion.subtabla == true || objConfiguracion.check == true || objConfiguracion.eliminar == true || objConfiguracion.editar == true || objConfiguracion.seleccionar == true) {
            ...
            if (objConfiguracion.check == true) {
                contenido += `<input type="checkbox" id='check-${fila[objConfiguracion.propiedadId]}' onclick='cambiarCheck(${fila[objConfiguracion.propiedadId]});' style='width:50px;' value='${fila[objConfiguracion.propiedadId]}' name='valor[]' checked>`;
            }
            ...
            if (objConfiguracion.check == true) {
                ...
                contenido += `<div id='text-${fila[objConfiguracion.propiedadId]}' >
                             <textarea name='obs[]' class='form-control' maxlength="150" style='height:40px;'>${obs}</textarea>
                             </div>`;
                contenido += "</td>";
                contenido += "<td>";
                contenido += `<div id='select-${fila[objConfiguracion.propiedadId]}'>
                                <select name='cant[]' id='option-${fila[objConfiguracion.propiedadId]}' class='form-select' style='width:60px; height:40px;'>`;
                if (cant == 1) {
                    contenido += `
            ...
            if (objConfiguracion.subtabla == true || objConfiguracion.check == true || objConfiguracion.eliminar == true || objConfiguracion.editar == true || objConfiguracion.seleccionar == true) {
                ...
                if (objConfiguracion.check == true) {
                    ...
                    contenido += "<td>";
                    contenido += `<div id='select-${fila[objConfiguracion.propiedadId]}'>
                                <select id='option-${fila[objConfiguracion.propiedadId]}'  class='form-select' style='width:60px;'  style='width:60px; height:40px;' disabled>
    ...
    if (validarDatos() == 0) {
        fetchPostText("Presupuesto/GuardarDatos", frm, function (res) {
            if (res != "0") {
                ...
                document.location.href = setUrl("Presupuesto/FrmPVistaEdit");
...
function Editar() {
    ...
    var id = document.getElementById("txtidRecepcion").value;

    /*recuperarGenerico("Cliente/RecuperarDatos/?id=" + id, "frmRecepción", ["Color"]);*/
    recuperarGenerico("Presupuesto/RecuperarDatos/?id=" + id, "frmRecepción", ["ID_Presupuesto", "ID_Vehiculo", "cant[]", "obs[]","valor[]"])
