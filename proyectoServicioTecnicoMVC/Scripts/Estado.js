...
function listar() {
    pintar({
        ...
        subtabla: false,
        colspanSub: 3,
        propiedades: [/*"ID_Marca",*/ "Nombre_Estado"],
        eliminar: true,
        editar: true,
        propiedadId: "ID_Estado"
    }, {
        busqueda: false,
        url: "Marca/filtrar",
...
function GuardarDatos() {
    ...
    var nombre = document.getElementById("txtEstado").value;

    var url = "Estado/VerificarEstado/?nombreEstado=" + nombre;
...
function GuardarDatosValidado() {
    var frmR = document.getElementById("formulario");
    var frm = new FormData(frmR);


    if (validarDatos() == 0) {
        fetchPostText("Estado/guardarDatos", frm, function (res) {
            if (res == "1") {
                listar();
                Limpiar();
...
function ModificarDatos() {
    ...
    url = "Estado/RecuperarDatos/?id=" + id;
...
function validarDatos() {
    ...
    if (document.getElementById("txtEstado").value == "") {
        document.getElementById("txtEstado").className = "form-control is-invalid";
        document.getElementById("invalid-Estado").innerHTML = 'Debe ingresar un Estado'
        val = 1;
        ...
        if (respuestaNombre.length != 0) {
            val = 1;
            document.getElementById("txtEstado").className = "form-control is-invalid";
            document.getElementById("invalid-Estado").innerHTML = 'El estado que intenta ingresar ya existe'

        } else {

            document.getElementById("txtEstado").className = "form-control is-valid";
        }
    }
...
function validarDatosM() {
    ...
    if (document.getElementById("txtEstado").value == "") {
        ...
        if (respuestaNombre.length != 0) {
            if (respuestaModificar.Nombre_Estado == respuestaNombre[0].Nombre_Estado) {
                document.getElementById("txtEstado").className = "form-control is-valid";
            } else {
                val = 1;
                document.getElementById("txtEstado").className = "form-control is-invalid";
...
function Limpiar() {
    ...
    LimpiarDatos("formulario", "ID_Estado", "");
...
function Editar(id) {
    ...
    removeVal()
    recuperarGenerico("Estado/RecuperarDatos/?id=" + id, "formulario");
...
function Eliminar(id) {
    fetchGetText("Estado/EliminarEstado/?id=" + id, function (res) {
        if (res == "1") {
            listar();
