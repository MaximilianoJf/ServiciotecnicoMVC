...
function listar() {
    pintar({
        ...
        id: "divTabla",
        cabeceras: [/*"ID", */ "Detalle", "Valor"],
        colspan: 3,
        subtabla: false,
        colspanSub: 3,
        propiedades: [/*"ID_Marca",*/ "Detalle", "Valor"],
        eliminar: true,
        editar: true,
...
function GuardarDatos() {
    ...
    var nombre = document.getElementById("txtDetalle").value;

    var url = "Reparaciones/VerificarReparacion/?nombreReparacion=" + nombre;
...
function GuardarDatosValidado() {
    ...
    if (validarDatos() == 0) {
        fetchPostText("Reparaciones/guardarDatos", frm, function (res) {
            if (res == "1") {
                listar();
                Limpiar();
...
function ModificarDatos() {
    ...
    var id = document.getElementById("txtIdEstado").value;
    url = "Reparaciones/RecuperarDatos/?id=" + id;
...
function validarDatos() {
    ...
    if (document.getElementById("txtDetalle").value == "") {
        document.getElementById("txtDetalle").className = "form-control is-invalid";
        document.getElementById("invalid-Detalle").innerHTML = 'Debe ingresar un detalle'
        val = 1;
        ...
        if (respuestaNombre.length != 0) {
            val = 1;
            document.getElementById("txtDetalle").className = "form-control is-invalid";
            document.getElementById("invalid-Detalle").innerHTML = 'El detalle que intenta ingresar ya existe'

        } else {

            document.getElementById("txtDetalle").className = "form-control is-valid";
        }
    }
...
function validarDatosM() {
    ...
    if (document.getElementById("txtDetalle").value == "") {
        ...
        if (respuestaNombre.length != 0) {
            if (respuestaModificar.Detalle == respuestaNombre[0].Detalle) {
                document.getElementById("txtDetalle").className = "form-control is-valid";
            } else {
                val = 1;
                document.getElementById("txtDetalle").className = "form-control is-invalid";
    ...
    if (document.getElementById("txtValor").value == "") {
        document.getElementById("txtValor").className = "form-control is-invalid";
        val = 1;

    } else {
        document.getElementById("txtValor").className = "form-control is-valid";
    }
...
function Limpiar() {
    ...
    LimpiarDatos("formulario", "ID_Reparacion", "");
...
function Editar(id) {
    ...
    removeVal()
    recuperarGenerico("Reparaciones/RecuperarDatos/?id=" + id, "formulario");
...
function Eliminar(id) {
    fetchGetText("Reparaciones/EliminarReparacion/?id=" + id, function (res) {
        if (res == "1") {
            listar();
